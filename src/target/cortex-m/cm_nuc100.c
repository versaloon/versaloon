...
#include "port.h"
#include "app_cfg.h"
#if TARGET_ARM_ADI_EN && TARGET_NUC100_EN
#include "app_type.h"
#include "app_io.h"
#include "app_err.h"
#include "app_log.h"
...
READ_TARGET_HANDLER(nuc100swj);
const struct program_functions_t nuc100swj_program_functions =
{
	NULL,
	ENTER_PROGRAM_MODE_FUNCNAME(nuc100swj),
	LEAVE_PROGRAM_MODE_FUNCNAME(nuc100swj),
	ERASE_TARGET_FUNCNAME(nuc100swj),
	WRITE_TARGET_FUNCNAME(nuc100swj),
...
#define NUC100_IAP_COMMAND_ADDR		(NUC100_IAP_BASE + NUC100_IAP_COMMAND_OFFSET)
#define NUC100_IAP_SYNC_ADDR		(NUC100_IAP_BASE + 0x74)
#define NUC100_IAP_CNT_ADDR			(NUC100_IAP_BASE + 0x78)
#define NUC100_IAP_FAIL_ADDR		(NUC100_IAP_BASE + 0x7C)
...
static vsf_err_t nuc100swj_unlock(void)
{
	uint32_t data[3] = {NUC100_REG_REGWRPROT_D0,
						NUC100_REG_REGWRPROT_D1,
						NUC100_REG_REGWRPROT_D2};
	
	if (adi_memap_write_reg32(NUC100_REG_REGWRPROT, &data[0], 0) ||
		adi_memap_write_reg32(NUC100_REG_REGWRPROT, &data[1], 0) ||
		adi_memap_write_reg32(NUC100_REG_REGWRPROT, &data[2], 1))
	{
		return VSFERR_FAIL;
	}
	return VSFERR_NONE;
...
static vsf_err_t nuc100swj_reset(void)
{
	uint32_t iprstc1 = NUC100_REG_IPRSTC1_CUP_RST;
	
	return adi_memap_write_reg32(NUC100_REG_IPRSTC1, &iprstc1, 1);
}

static vsf_err_t nuc100swj_reset_to_aprom(void)
{
	uint32_t ispcon = NUC100_REG_ISPCON_BS_APROM;
	
	if (adi_memap_write_reg32(NUC100_REG_ISPCON, &ispcon, 1) ||
		nuc100swj_reset())
	{
		return VSFERR_FAIL;
	}
...
static vsf_err_t nuc100swj_reset_to_ldrom(void)
{
	uint32_t ispcon = NUC100_REG_ISPCON_BS_LDROM;
...
WRITE_TARGET_HANDLER(nuc100swj)
{
	...
	case APPLICATION_CHAR:
		...
		cmd.command = NUC100_REG_ISPCMD_PROGRAM;
		cmd.cnt = page_size / 4;
		while (size)
		{
...
READ_TARGET_HANDLER(nuc100swj)
{
	...
	{
	case CHIPID_CHAR:
		if (adi_memap_read_reg32(NUC100_REG_PDID, &pdid, 1))
		{
